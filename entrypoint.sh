...
LB_OPTS="$LB_OPTS --defaultsFile=/liquibase.properties"
...
## Database url
if [[ -n "$LB_URL" ]]; then
    echo "url: ${LB_URL}" >> /liquibase.properties
fi

## Database username
if [[ -n "$LB_USER" ]]; then
    echo "username: ${LB_USER}" >> /liquibase.properties
fi

## Database password
if [[ -n "$LB_PASS" ]]; then
    echo "password: ${LB_PASS}" >> /liquibase.properties
fi

## Database contexts
if [[ -n "$LB_CONTEXTS" ]]; then
    echo "contexts: ${LB_CONTEXTS}" >> /liquibase.properties
fi

## Database changelog file
if [[ -n "$LB_CHANGELOG" ]]; then
    echo "changeLogFile: ${LB_CHANGELOG}" >> /liquibase.properties
fi

## Database changelog file
if [[ -n "$LB_LOGLEVEL" ]]; then
    echo "logLevel: ${LB_LOGLEVEL}" >> /liquibase.properties
fi

function executeLiquibase() {
    exec liquibase $LB_OPTS "$@"
}

function executeLiquibaseCommonInfo() {
    exec liquibase "$@"
}
